...
void WoundWaitBasicTest() {
  ...
  for (size_t i = 0; i < num_kill; i++) {
    std::cout<< "killing begin"<<std::endl;
    kill_threads.emplace_back(std::thread{kill_task});
  }

  for (size_t i = 0; i < num_wait; i++) {
    wait_threads[i].join();
    std::cout<< "size is after join,"<<lock_mgr.lock_table_[rid0].request_queue_.size() << std::endl;
...
std::cout<< "size is "<<lock_mgr.lock_table_[rid0].request_queue_.size() << std::endl;
