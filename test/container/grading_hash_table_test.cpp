...
TEST(HashTableTest, IntegratedConcurrencyTest) {
  const int num_threads = 30;// originally 5
  const int num_runs = 200;// originally 50
  ...
  for (int run = 0; run < num_runs; run++) {
    ...
    for (int tid = 0; tid < num_threads; tid++) {
      threads[tid] = std::thread([&ht, tid]() {
        ...
        //std::cout<<"partial success, thread is "<< tid << std::endl;
        EXPECT_EQ(tid, res[0]);
       // std::cout<<"insert success, thread is "<< tid << std::endl;
